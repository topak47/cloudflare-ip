...
hostmonit() {
    echo "抓取 https://api.hostmonit.com/get_optimization_ip"
    resp=$(curl -s -X POST -H "Content-Type: application/json" -d '{"key": "iDetkOys"}' https://api.hostmonit.com/get_optimization_ip)
    
    # 清空之前的CSV文件内容并写入表头
    echo "IP地址,线路" > "$output_file"

    # 逐行解析JSON响应，提取线路和IP
    while read -r line; do
        ip=$(echo "$line" | jq -r '.ip')
        line_type=$(echo "$line" | jq -r '.line')
        
        # 如果线路在预设线路列表中，则写入CSV文件
        if [[ " ${preset_lines[@]} " =~ " $line_type " ]]; then
            echo "$ip,$line_type" >> "$output_file"
        fi
    done <<< $(echo "$resp" | jq -c '.info[]')
    
    echo "IP地址已写入 $output_file"
}
